#!/usr/bin/env Rscript
# Phase 3 per-block R² map — same pattern as Phase 2's build_block_r2_map.R
# but points at P3 parquet + p3 model cache. Uses all 15 Full-model .rds
# (5 folds × 3 PFTs) to produce a 3-panel choropleth.
...
message("Reading Phase 3 parquet")
...
write.csv(block_r2, file.path(P2_RESULT_DIR, "p3_block_r2.csv"), row.names = FALSE)
message("\nSaved per-block R² -> p3_block_r2.csv (", nrow(block_r2), " rows)")
...
message("\nPer-block R² summary by PFT (Phase 3 Full, 20 predictors):")
print(summary_df)
...
p <- ggplot(block_r2,
  ...
  labs(
    title = expression(Phase~3~block-CV~R^2~per~5*degree~block~"("*Full~20-predictor~model~"with"~ice~export*")"),
...
ggsave(file.path(P2_RESULT_DIR, "p3_block_r2_map.png"), p,
       width = 13, height = 5, dpi = 300, bg = "white")
message("\nSaved map -> p3_block_r2_map.png")
