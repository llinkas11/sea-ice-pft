#!/bin/bash
#SBATCH --job-name=rf_p3t_Phaeocystis
...
echo "P3 temporal CV Phaeocystis started: $(date) on $(hostname)"
echo "SLURM_CPUS_ON_NODE=$SLURM_CPUS_ON_NODE  SLURM_JOB_ID=$SLURM_JOB_ID"
export P3_PFT_FILTER=Phaeocystis
Rscript "$RF_PROJECT_ROOT/R/model_comparison_p3_temporal.R"
echo "P3 temporal CV Phaeocystis finished: $(date)"
